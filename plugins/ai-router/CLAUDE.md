...
>
> **Per-host opt-out gate.** Once you've opted out (or accept the policy) on a given machine, mark Sakana safe **on that host** with `touch ~/.config/ai-router/sakana-notraining-confirmed` (any content) or `export AI_ROUTER_SAKANA_NOTRAINING=1`. `sakana_status` / `or_status` / the `sakana_ask` header then report **"no-training confirmed on this host"** instead of the warning (`go/routing.go:sakanaNoTrainingConfirmed`, mirroring the GPT-5.5 gate). The committed default stays the warning — this gate is host-local and never committed, so marketplace users aren't misled. Remove the file / unset the env to restore the warning.
